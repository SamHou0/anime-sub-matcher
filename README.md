...
一个简单易用的 Web 应用程序，用于自动识别字幕文件的集数并匹配到对应的剧集视频文件。**支持批量上传整个季度的字幕！**

## ✨ 特性

- 🎯 **自动识别集数**: 使用 anitopy 库智能解析各种动画字幕命名格式
- 📦 **批量上传**: 支持一次上传整个季度的所有字幕文件
- 🚀 **批量匹配**: 自动批量处理所有字幕，一键完成整季匹配
...
**Q: 支持批量上传吗?**  
A: 支持！可以一次选择整个季度的所有字幕文件，系统会自动批量处理和匹配。
...
**Q: 批量匹配时如果部分文件失败怎么办?**  
A: 界面会显示每个文件的匹配状态，成功的文件会被匹配，失败的会显示错误信息。
